...
int main()
{
    ...
    //FILE *fpout = fopen("change/change3.out","w");
    long n,m;
    fscanf(fp,"%ld %ld",&n,&m);
    ...
    *prefixProduct = 1;
    ...
    for(int i = 0;i < m;i++)
    {
        fscanf(fp,"%ld %ld %lf %lf",&dots[i].start,&dots[i].end,&dots[i].x,&dots[i].y);
    }
    for(long i = STOP - 10;i < m;i++)
    {
        printf("\n/////////////////%d//////////////////\n",i + 1);
        long j,k,mode1,mode2;
        dot d;
        d.x = dots[i].x,d.y = dots[i].y;
        j = dots[i].start,k = dots[i].end;
        printf("x = %.6lf,y = %.6lf\n",d.x,d.y);
        j--;k--;
        long start1 = j;
        long end1 = k;
