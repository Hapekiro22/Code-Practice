...
void printList(block **blocks,FILE *fout);

long long maxNum = 0;
void dfs(block **blocks, long long curNum,int count,int *curLoc,FILE *fout);

int main()
{
    FILE *fin = fopen("evaldata/p1012_2.in","r");
    FILE *fout = fopen("evaldata/p1012.out","w");
    ...
    dfs(blocks,curNum,count,curLoc,fout);
    printf("%lld",maxNum);
    fprintf(fout,"%lld",maxNum);
...
void dfs(block **blocks, long long curNum,int count,int *curLoc,FILE *fout)
{
    ...
    for(int i = 1;i < MAX_NUMLEN;i++)
    {
        ...
        *(curLoc + i) = tempLoc + 1;
        ...
        {
            if(1)
            fprintf(fout,"Now curNum = %lld, try to add %lld\n",curNum,tempNum);
        }
...
void printList(block **blocks,FILE *fout)
{
    for (int i = 0; i < MAX_NUMLEN; i++)
    {
        ...
        for (int j = 0; ;j++)
        {
            ...
            fprintf(fout,"%lld  ",blocks[i][j].num);
        }
        fprintf(fout,"\n");
    }
    return;
