//插入排序
//添加功能6.文件保存功能：将链表结点中学生的各项信息保存到二进制文件stu.dat
...
void SaveList(student *head)
{
    ...
    while(p)
    {
        fwrite(p->id,sizeof(char),20,fp);
        fwrite(p->name,sizeof(char),50,fp);
        fwrite(p->score,sizeof(int),4,fp);
        fwrite(&p->average,sizeof(double),1,fp);
        p = p->next;
    }
    fclose(fp);
    return;
...
void OpenList(student *head)
{
    ...
    while(p)
    {
        fread(p->id,sizeof(char),20,fp);
        fread(p->name,sizeof(char),50,fp);
        fread(p->score,sizeof(int),4,fp);
        fread(&p->average,sizeof(double),1,fp);
        p = p->next;
    }
    fclose(fp);
}
